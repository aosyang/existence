...
void FileSystem::FindFilesInDir(const String& basePath, vector<FileNameInfo>& foundFileList, bool recursive, const String& subPath)
{
	//String fullName = basePath + CORRECT_SLASH + filename;
	...
	do
	{
		...
		else
		{
			...
			nameinfo.path = basePath;
			nameinfo.filename = subPath + info.filename;
			nameinfo.ext = GetExtension(info.filename);
			nameinfo.ext.ToLowerCase();

			// 这将防止材质优先于纹理载入
			if (nameinfo.ext==".emt")
				nameinfo.period = 2;
			else if (nameinfo.ext==".emd")
				nameinfo.period = 3;
			else
				nameinfo.period = 0;

			foundFileList.push_back(nameinfo);
		}
