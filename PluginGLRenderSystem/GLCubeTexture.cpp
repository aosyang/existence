...
void GLCubeTexture::Create(unsigned int width, unsigned int height, unsigned int bpp, unsigned char* data, int cubePos)
{
	...
	if (GLTexture::s_ForcePowOfTwo && (width!=pow_of_two_width || height!=pow_of_two_height))
	{
		...
		gluScaleImage(m_Format[cubePos], width, height, GL_UNSIGNED_BYTE, data, m_Width, m_Height, GL_UNSIGNED_BYTE, scaleImage);

		glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + cubePos, 0, m_InternalFormat[cubePos], m_Width, m_Width,
